...
var config = {
    ...
    buildSequence: ['images:resize-sprite-source', 'css:lazy-rules', 'css:import-modules', 'css:compile', 'images:sprites', 'build:copy', 'build:include', 'images:optimize'],
...
var modules = {
    ...
    css         : function() {
        ...
        var taskMinifyCSS = function() {
            var cssFilePath = config.paths.build.path + config.paths.dist.css.replace('.', '') + '/';
            var task        = gulp
                                .src(cssFilePath + '*.css')
                                .pipe(cleanCSS())
                                .pipe(rename({
                                ...
                                .pipe(gulp.dest(cssFilePath));

            return task;
        };

        var taskReplaceCSS = function() {
            var task = gulp
                        .src(config.paths.build.path + '/*.html')
        ...
        var register = function() {
            ...
            gulp.task('css:minifyCSS', taskMinifyCSS);
            gulp.task('css:replaceCSS', taskReplaceCSS);
        };
    ...
    javascript  : function() {
        var taskMinifyJS = function() {
            var jsFilePath = config.paths.build.path + config.paths.dist.js.replace('.', '') + '/';
            var task       = gulp
                                .src(jsFilePath + '*.js')
                                .pipe(uglify())
                                .pipe(rename({
        ...
        var taskReplaceJS = function() {
            var task = gulp
                        .src(config.paths.build.path + '/*.html')
                        .pipe(replace('functions.js', 'functions.min.js'))
        ...
        var register = function() {
            gulp.task('js:minifyJS', taskMinifyJS);
            gulp.task('js:replaceJS', taskReplaceJS);
        };
    ...
    es6  : function() {
        ...
        var init = function() {
            watch(config.paths.source.es6 + '/**/*.js', function() {
    ...
    html        : function() {
        ...
        var register = function() {
            gulp.task('html:minifyHTML', taskMinifyHTML);
        };
    ...
    build       : function() {
        ...
        var taskBuildCopy = function() {
            return gulp.src(['**', '!~QA/**', '!partials/**', '!src/**/*', '!package.json', '!package-lock.json', '!yarn.lock', '!settings.json', '!peon.json', '!drone.json', '!README.md', '!gulpfile.js', '!' + config.paths.source.sprite + '/*', '!' + config.paths.source.css + '/*', '!**/*.map', '!build', '!node_modules/**/*', '!*.html', '!package-lock.json', '!yarn.lock', '!node_modules/**/*'])
                .pipe(copy('build/'));
        };
        ...
        var taskBuildInclude = function() {
            return gulp.src(['**/*.html', '!node_modules/**/*.html', '!build/**/*.html', , '!node_modules/**/*.html'])
...
gulp.task('build', ['build:clean'], function() {
    ...
    if (argv.mincss) {
        config
            .buildSequence
                .push('css:minifyCSS', 'css:replaceCSS');
    }

    if (argv.minjs) {
        config
            .buildSequence
                .push('js:minifyJS', 'js:replaceJS');
    }

    if (argv.minhtml) {
        config
            .buildSequence
                .push('html:minifyHTML');
    }

    runSequence(...config.buildSequence);
});
